...
class MyTracePlugin < ThroughPlugin
...
EOT
    ...
    #-------------   check restrict ---------#
    print "callable from #{@caller_cell.get_namespace_path} " +
          "to #{@callee_cell.get_namespace_path}.#{@join.get_port_name}.#{func_name} " +
          "= #{@caller_cell.callable?( @callee_cell, @join.get_port_name, func_name )}\n"
